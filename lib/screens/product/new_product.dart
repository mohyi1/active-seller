...
class _NewProductState extends State<NewProduct> {
  ...
  setConstDropdownValues() {
    ...
    shippingConfigurationList.addAll([
      CustomRadioModel("ًالشحن مجانا", "free", true),
      CustomRadioModel("سعر ثابت", "flat_rate", false),
    ]);
    //////// HErE
    stockVisibilityStateList.addAll([
      CustomRadioModel("إظهار كمية المخزون", "quantity", true),
      CustomRadioModel("عرض المخزون مع النص فقط", "text", false),
      CustomRadioModel("إخفاء المخزون", "hide", false)
    ]);
  ...
  getTaxType() async {
    ...
    taxRes.data!.forEach((element) {
      vatTaxList.add(
        ...
        CommonDropDownItem("amount", "قيمة ثابت"),
        CommonDropDownItem("percent", "النسبة المئوية"),
      ]));
    });
  }
  ...
  bool requiredFieldVerification() {
    if (productNameEditTextController.text.trim().toString().isEmpty) {
      ToastComponent.showDialog("اسم المنتج مطلوب", gravity: Toast.center);
      return false;
    } else if (selectedCategory == null) {
      ToastComponent.showDialog("فئة المنتج المطلوبة",
          gravity: Toast.center);
      return false;
    } else if (minimumEditTextController.text.trim().toString().isEmpty) {
      ToastComponent.showDialog("الحد الأدنى لكمية المنتج المطلوبة",
          gravity: Toast.center);
      return false;
    } else if (unitEditTextController.text.trim().toString().isEmpty) {
      ToastComponent.showDialog("وحدة المنتج المطلوبة", gravity: Toast.center);
      return false;
    }
    return true;
  }
